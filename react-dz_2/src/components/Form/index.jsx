...
export default class AddTodoForm extends Component {
    ...
    _handleSubmit = evt => {
        ...
        this.setState({inputValue: '', inputArea: '', rating: '1'});
    };

    render() {
        const {inputValue, inputArea, rating} = this.state;

        return (<form onSubmit={this._handleSubmit} className="MovieForm">
            <label className="MovieForm__label">Title
                <input type="text" value={inputValue} onChange={this._handleInputChange} className="MovieForm__input"/>
            </label>
            <label className="MovieForm__label">Description
                <textarea value={inputArea} onChange={this._handleInputAreaChange} className="MovieForm__txtarea" rows="8"></textarea>
            </label>
            <label className="MovieForm__label">Rating
                <select value={rating} onChange={this._handleRatingChange} className="MovieForm__input">
